...
def parse_snr_values(raw: str | None) -> list[float]:
    if raw is None or raw.strip() == "":
        return [float(x) for x in range(10, 21, 2)]
...
def format_snr_value(snr_db: float) -> str:
    ...
    return f"{snr_db:g}".replace(".", "p")


def write_results_csv(path: Path, results: list[dict[str, float | int | str]]) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    fieldnames = ["mode", "snr_db", "loss", "pm", "pf", "p_mean", "epoch", "save_dir"]
...
def plot_loss_vs_snr(
    ...
    try:
        ...
        write_loss_vs_snr_svg(svg_path, results_by_mode)
    ...
    for mode, rows in results_by_mode.items():
        if not rows:
            continue
        xs = [float(row["snr_db"]) for row in rows]
        ys = [float(row["loss"]) for row in rows]
        plt.plot(xs, ys, marker="o", linewidth=2.0, label=mode)
    plt.xlabel("SNR (dB)")
    plt.ylabel("loss")
    plt.title("Final Training Loss vs SNR")
    plt.xlim(10, 20)
    plt.xticks(list(range(10, 21, 2)))
...
def write_loss_vs_snr_svg(
    ...
    lines: list[str] = [
        ...
        '<text x="450" y="28" text-anchor="middle" font-family="Arial" font-size="20">Final Training Loss vs SNR</text>',
    ...
    lines.extend(
        [
            '<text x="450" y="535" text-anchor="middle" font-family="Arial" font-size="16">SNR (dB)</text>',
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--append-log", action="store_true", help="Append to existing per-SNR train.log files.")
...
def main() -> None:
    cli = parse_args()
    snr_values = parse_snr_values(cli.snrs)
    ...
    csv_path = cli.out_dir / "final_loss_by_SNR.csv"
    plot_path = cli.out_dir / "final_loss_by_SNR.png"
    ...
    for mode in modes:
        for snr_db in snr_values:
            ...
            snr_label = format_snr_value(float(snr_db))
            run_dir = cli.out_dir / mode / f"SNR{snr_label}"
            args.save_dir = run_dir
            args.log_file = run_dir / "train.log"
            if args.log_file.exists() and not cli.append_log:
                args.log_file.unlink()

            print(f"\n=== Running {mode}: SNR={snr_db:g} dB, epochs={args.epochs}, save_dir={run_dir} ===")
            ...
            last_plot_path = plot_loss_vs_snr(plot_path, results_by_mode, cli.dpi)
            print(f"Recorded {mode} SNR={snr_db:g} dB: loss={row['loss']:.6f}")
