...
def parse_pilot_lens(raw: str | None) -> list[int]:
...
def plot_loss_vs_l(
    ...
    try:
        ...
        write_loss_vs_l_svg(svg_path, results_by_mode)
    ...
    for mode, rows in results_by_mode.items():
        if not rows:
            continue
        xs = [int(row["pilot_len"]) for row in rows]
        ys = [float(row["loss"]) for row in rows]
        plt.plot(xs, ys, marker="o", linewidth=2.0, label=mode)
    plt.xlabel("Pilot length L")
    plt.ylabel("loss")
    plt.title("Final Training Loss vs Pilot Length")
    plt.xlim(4, 30)
    plt.xticks(list(range(4, 31, 2)))
...
def write_loss_vs_l_svg(
    ...
    lines: list[str] = [
        ...
        '<text x="450" y="28" text-anchor="middle" font-family="Arial" font-size="20">Final Training Loss vs Pilot Length</text>',
    ...
    lines.extend(
        [
            '<text x="450" y="535" text-anchor="middle" font-family="Arial" font-size="16">Pilot length L</text>',
...
def main() -> None:
    cli = parse_args()
    pilot_lens = parse_pilot_lens(cli.pilot_lens)
    ...
    csv_path = cli.out_dir / "final_loss_by_L.csv"
    plot_path = cli.out_dir / "final_loss_by_L.png"
    last_plot_path: Path | None = None

    for mode in modes:
        for pilot_len in pilot_lens:
            args = copy(build_args())
            args.pilot_len = int(pilot_len)
            ...
            last_plot_path = plot_loss_vs_l(plot_path, results_by_mode, cli.dpi)
            print(f"Recorded {mode} L={pilot_len}: loss={row['loss']:.6f}")
